...
namespace UnityGameFramework.Runtime
{
    ...
    public class DefaultLocalizationHelper : LocalizationHelperBase
    {
        ...
        private Dictionary<Language, int> ParseLanguageIndex(string dictionaryLineString)
        {
            Dictionary<Language, int> languageDictionary = new();
            string[] languages = dictionaryLineString.Split(ColumnSplitSeparator, StringSplitOptions.None);
            for (int i = 2; i < languages.Length; i++)
            {
                if (Enum.TryParse<Language>(languages[i], true, out Language language))
                {
