...
namespace Quester
{
    public abstract class UGuiForm : UIFormLogic
    {
...
#endif
        {
            ...
            if (faderPrefab)
            {
                var fader = Instantiate(faderPrefab, transform);
                fader.transform.SetAsFirstSibling();
            }
            ...
            TMP_Text[] textsTmp = GetComponentsInChildren<TMP_Text>(true);
            for (int i = 0; i < textsTmp.Length; i++)
            {
                // textsTmp[i].font = _tmpFontAsset;
                var value = textsTmp[i].text;
                if (!string.IsNullOrEmpty(value))
                {
                    textsTmp[i].text = GameEntry.Localization.GetString(textsTmp[i].text);
                }
            }
