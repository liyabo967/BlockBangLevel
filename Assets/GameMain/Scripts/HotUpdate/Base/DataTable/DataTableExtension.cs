...
namespace Quester
{
    ...
    {
        private const string DataRowClassPrefixName = "Quester.DR";
		private const string PlotLanguageName = "PlotLanguage";
		...
		public static void LoadDataTable(this DataTableComponent dataTableComponent, string dataTableName, string dataTableAssetName, object userData)
		{
			...
			string name = splitedNames.Length > 1 ? splitedNames[0] + splitedNames[1] : null;
			DataTableBase dataTable = dataTableComponent.CreateDataTable(dataRowType, name);
			dataTable.ReadData(dataTableAssetName, Constant.AssetPriority.DataTableAsset, userData);
		}

		public static void LoadPlotLanguage(this DataTableComponent dataTableComponent, string dataTableName, string dataTableAssetName, object userData)
		{
			if (string.IsNullOrEmpty(dataTableName))
			{
				Log.Warning("Data table name is invalid.");
			...
			string dataRowClassName = DataRowClassPrefixName + PlotLanguageName;
			Type dataRowType = Type.GetType(dataRowClassName);
			if (dataRowType == null)
			{
				Log.Warning("Can not get data row type with class name '{0}'.", dataRowClassName);
			...
			DataTableBase dataTable = dataTableComponent.CreateDataTable(dataRowType, dataTableName);
			dataTable.ReadData(dataTableAssetName, Constant.AssetPriority.DataTableAsset, userData);
		}
		// public static bool HasPlotLanguage(this DataTableComponent dataTableComponent, string dataTableName, string dataTableAssetName, object userData)
		// {
		// 	if (string.IsNullOrEmpty(dataTableName))
		// 	{
		// 		Log.Warning("Data table name is invalid.");
		...
		// 		return false;
		// 	}
		// 	if (dataTableComponent.HasDataTable(dataRowType, dataTableName))
		// 	{
		// 		DataTableBase dataTable = dataTableComponent.GetAndReadDataTable(dataRowType, dataTableName);
		// 		dataTable.ReadData(dataTableAssetName, Constant.AssetPriority.DataTableAsset, userData);
		// 		return true;
		// 	}
		// 	return false;
